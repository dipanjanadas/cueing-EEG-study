...
load(sprintf('distCueTFdata5subs.mat'));

%rt data
observedData=newDistractorCueData(:,1:6);
...
observedRTDataNew(:,4) = observedData(:,2); %incongruent valid
...
statOuputTarget=teg_repeated_measures_ANOVA(observedRTDataNew,[2,3],{'congruence','cue_type'})

%valid, invalid-other
newAC(:,1)= observedRTDataNew(:,1);%congValid
newAC(:,2)=observedRTDataNew(:,2);%congInvalid 
newAC(:,3)=observedRTDataNew(:,4);%incong valid
newAC(:,4)=observedRTDataNew(:,5);%incong invalid

%vlaid, invalid-target
newAC1(:,1)= observedRTDataNew(:,1);%congValid
newAC1(:,2)=observedRTDataNew(:,3);%cong np
newAC1(:,3)=observedRTDataNew(:,4);%incong valid
newAC1(:,4)=observedRTDataNew(:,6);%incong np

%invalid other, invalid-target
newAC2(:,1)= observedRTDataNew(:,2);%congValid
newAC2(:,2)=observedRTDataNew(:,3);%cong np
newAC2(:,3)=observedRTDataNew(:,5);%incong valid
newAC2(:,4)=observedRTDataNew(:,6);%incong np



statOuput=teg_repeated_measures_ANOVA(observedRTDataNew,[2,3],{'congruence','cue_type'})
%only a main effect of congruence
statOuputT1=teg_repeated_measures_ANOVA(newAC1,[2,2],{'congruence','cue_type'})

statOuput2=teg_repeated_measures_ANOVA(newAC2,[2,2],{'congruence','cue_type'})

newAC2(:,1)=observedRTDataNew(:,1);%CONG VALID 
newAC2(:,2)=mean(observedRTDataNew(:,2:3),2);%TOT CONG INVALID
newAC2(:,3)= observedRTDataNew(:,4);% INCONG VALID 
newAC2(:,4)=mean(observedRTDataNew(:,5:6),2);%TOT INCONG INVALID

statOuput3=teg_repeated_measures_ANOVA(newAC2,[2,2],{'congruence','cue_type'})

%% Accuracy data cleaning 

%removing AC outliers: removed all subjects with any condition <=.5
allSjPCdata=distractorCueData(:,7:12);
...
observedACData(:,4) = dataPC(:,2); %incongruent valid
observedACData(:,5) = dataPC(:,4); %incongruent invalid
observedACData(:,6) = dataPC(:,6); %incongruent negative priming
% observedACData(:,8) = dataPC(:,8); %incongruent neutral


%valid, invalid-other
newAC(:,1)= observedACData(:,1);%congValid
newAC(:,2)=observedACData(:,2);%congInvalid 
newAC(:,3)=observedACData(:,4);%incong valid
newAC(:,4)=observedACData(:,5);%incong invalid

%vlaid, invalid-target
newAC1(:,1)= observedACData(:,1);%congValid
newAC1(:,2)=observedACData(:,3);%cong np
newAC1(:,3)=observedACData(:,4);%incong valid
newAC1(:,4)=observedACData(:,6);%incong np

% statOuput=teg_repeated_measures_ANOVA(observedDataNew,[2,3],{'congruence','cue_type'})
%only a main effect of congruence
statOuput1=teg_repeated_measures_ANOVA(newAC,[2,2],{'congruence','cue_type'})

statOuput2=teg_repeated_measures_ANOVA(newAC1,[2,2],{'congruence','cue_type'})

newAC2(:,1)=observedACData(:,1);%CONG VALID 
newAC2(:,2)=mean(observedACData(:,2:3),2);%TOT CONG INVALID
newAC2(:,3)= observedACData(:,4);% INCONG VALID 
newAC2(:,4)=mean(observedACData(:,5:6),2);%TOT INCONG INVALID
...
statOutputAC=teg_repeated_measures_ANOVA(observedACData,[2,3],{'congruence','cue_type'})
