function analysisCode5distCue(sublist) % Lena, you can type in analysisCode4([# #]) in the command window 
...
for sub=1:length(sublist)
    load(sprintf('sj%02d_distCueTF.mat',sublist(:,sub)))
    %valid trials
    congCountValid = 0;
    incongCountValid = 0;
...
%     totIncongNeut=nIncongNeut(:,1);
    ...
    %trials.
%     deleteCongValidRT2=find(congVecValid(:,1)<200);
...
%     deleteIncongValidRT2=[find(incongVecValid(:,1)<200)];
...
%     deleteCongNPRT3=  [find(congVecNP(:,1)<200)];
...
%     deleteIncongNPRT3=[find(incongVecNP(:,1)< 200)]; 
...
%     incongNeutPC=sum((incongNeutAcc)/length(totIncongNeut));
    ...
    distractorCueData(sub,1)=meanCongValidRT;
    distractorCueData(sub,2)=meanIncongValidRT;
    distractorCueData(sub,3)=meanCongInvalidRT;
    distractorCueData(sub,4)=meanIncongInvalidRT;
    distractorCueData(sub,5)=meanCongNPRT;
    distractorCueData(sub,6)=meanIncongNPRT;
%     distractorCueData(sub,7)=meanCongNeutRT;
%     distractorCueData(sub,8)=meanIncongNeutRT;
    distractorCueData(sub,7)=congValidPC;
    distractorCueData(sub,8)=incongValidPC;
    distractorCueData(sub,9)=congInvalidPC;
    distractorCueData(sub,10)=incongInvalidPC;
    distractorCueData(sub,11)=congNPPC;
    distractorCueData(sub,12)=incongNPPC;
%     distractorCueData(sub,15)=congNeutPC;
%     distractorCueData(sub,16)=incongNeutPC;
    
    save('datadistcuejune26', 'distractorCueData');
...
%     data(sub).incongNeutPC=incongNeutPC;
    ...
    save(sprintf('%s02d_distCueTFDataJune26', 'sub'),'data');
