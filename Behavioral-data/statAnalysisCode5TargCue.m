filename='targCueBehTF.xlsx'; %congValid, incongValid, congInvalid, incongInvalid...
distractorCueData=xlsread(filename);

%rt data
observedData=newTargetCueData(:,7:12);
...
%valid, invalid-other
newRT(:,1)= observedRTDataNew(:,1);%congValid
newRT(:,2)=observedRTDataNew(:,2);%congInvalid 
newRT(:,3)=observedRTDataNew(:,4);%incong valid
newRT(:,4)=observedRTDataNew(:,5);%incong invalid

%vlaid, invalid-target
newRT1(:,1)= observedRTDataNew(:,1);%congValid
newRT1(:,2)=observedRTDataNew(:,3);%cong np
newRT1(:,3)=observedRTDataNew(:,4);%incong valid
newRT1(:,4)=observedRTDataNew(:,6);%incong np

%invalid other, invalid target
newRT2(:,1)= observedRTDataNew(:,2);%cong inValid
newRT2(:,2)=observedRTDataNew(:,3);%cong np
newRT2(:,3)=observedRTDataNew(:,5);%incong invalid
newRT2(:,4)=observedRTDataNew(:,6);%incong np


statOuput1=teg_repeated_measures_ANOVA(newRT2,[2,2],{'congruence','cue_type'})
%
%
%
...
allSjPCdata=newTargetCueData(:,7:12);
