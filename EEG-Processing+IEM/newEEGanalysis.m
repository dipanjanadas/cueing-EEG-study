function neweEEGanalysis(fName,root,permute)
...
fName='sj07_distCueTF.set';
eegdir = '/eegSetFiles';
behdir = '/beh';
root=pwd;

eegdir = '/eegSetFiles';
eegEpoched2CueDir = '/epoched2Cue_newEEGanalysisDist2';
% eegEpoched2StimDir = 'eegEpoched2StimNew';
% eegEpochedFiltDir= 'eegEpochedFilt';
% eegEpoched2Dir='/eegEpoched2';
behdir = '/beh';
iemdir = '/iem_newEEGanalysisDist2';
% stimlockiemdir='/iemStim_newCodes';

 if strcmp(fName(6:9),'dist')
%load in .set files
...
if strcmp(fName(4:9),'7_dist')
    brokenFix = [trialLog.trialInfo([2:1472]).brokenTrial]';
end
...
%for sj 7 after clearing out broken fix data i had to remove two additonal
% %beh log trials and one additional epoch from the data to line up the data.
% removedata=zeros(1,1319);
% removedata(1,1)=1;
% EEG=pop_rejepoch(EEG, removedata, 0); 
% trialLogNew(1:2) = [];
...
for iTrial=1:length(trialLogNew)
    ...
    if trialLogNew(iTrial).cue == trialLogNew(iTrial).distractor
...
%         EEG.event(i).cueType=EEG.event(i).type
%     elseif EEG.event(i).type==52
%         EEG.event(i).cueType=EEG.event(i).type
%     elseif EEG.event(i).type==53
%         EEG.event(i).cueType=EEG.event(i).type
%     elseif EEG.event(i).type==54
%         EEG.event(i).cueType=EEG.event(i).type
%     elseif EEG.event(i).type==55
%         EEG.event(i).cueType=EEG.event(i).type
%     elseif EEG.event(i).type==56
%         EEG.event(i).cueType=EEG.event(i).type
%     end
% end 
% 
% cueType=[EEG.event.cueType]';
% cueType1=cueType;
% cueType1=[cueType1(:,1)-50];
% % Create C1
% C1= [];
% for i = 1:size(cueType1,1)
%     C1(i,:) = call(cueType1(i,:),:);
% end
% % Leave-one-out cross validation definition
% tpart = cvpartition(cueType1(:,1),'Leaveout');
