function newEEGanalysisBase(fName,root,permute)
...
fName='sj02_baseCueTF.set';
% eegdir = '/eegSetFiles2';
behdir = '/beh';
root=pwd;

eegdir = '/eegSetFiles2';
eegEpoched2CueDir = '/epoched2cue_newEEGanalysisBase3';      %'/epoched2Cue_newEEGanalysisBase1';
...
% % 
% if strcmp(fName(4:9),'7_base')
%     brokenFix = [trialLog.trialInfo([155:1573]).brokenTrial]';
% end
% 
% if strcmp(fName(4:9),'5_base')
%     brokenFix = [trialLog.trialInfo([1:218 233:1532]).brokenTrial]';
% end
% 
% if strcmp(fName(4:9),'6_base')
...
for iTrial=1:length(trialLogNew)
    ...
    if trialLogNew(iTrial).trialType==200
...
EEG= pop_epoch(EEG, {'50'}, [-0.2  1.12], 'newname', 'BDF file epochs', 'epochinfo', 'yes');
%[-.2 1.25] new epoch to try  %was [-.2 1]
%%correct for baseline
EEG = pop_rmbase( EEG, [-200  0]); 
...
%upload EEG file
% EEG = pop_loadset([pwd eegEpoched2CueDir  '/' fName]);
% data = load([root '/' eegEpoched2CueDir '/' fName(1:14) '.mat']);
% EEG = data.EEG;
% data = [];
...
% if strcmp(fName(4:9),'2_base') %do sj 2 by itself  
   hilbertEEG([15 19],:,:)=[]; 
% end 
...
for iTrial=1:length(trialLogNew)
    if trialLogNew(iTrial).accuracy==1
        if trialLogNew(iTrial).trialType==200
            ...
            locVec(cnt) = trialLogNew(iTrial).distractor;
...
locVec=locVec';
locVec= locVec(randperm(length(locVec)));
...
for i = 1:size(hilbertEEG,2) %52:size(hilbertEEG,2)
    % Define EEG (observed data)
        ...
        B = squeeze(hilbertEEG(1:62,i,:)); %change back to 64 %for sj 2, it should be 1:62 channels. 
%     B = squeeze(hilbertEEG(1:64,i,:));%change back to 1:64 after being done with sj2_base testing
