...
trialLog(1).experiment='mainBaselineTFExp';
trialLog(1).age=age;
...
trialMatrix=[num2cell(dist); num2cell(targ); num2cell(trialType)]';
...
% trialMatrixCheck=[cue;dist;targ;congMatrix;tOrient;dOrient;trialType]';
trialMatrixCheck=[targ;dist;congMatrix;tOrient;dOrient;trialType]';
...
if eyetracker == 1
    EyelinkInit([], 1);
    edfFile = [num2str(sjNum) '_' 'bs' '.edf']; % Name of eyetracking data file - has to be in the working directory unfortunately
...
trialMatrix=[targ;dist;congMatrix;tOrient;dOrient;trialType];
...
%     Screen('Flip',window);
 ...
 % block message
    thisMessage = sprintf('This is Block %d of %d',thisBlock, nBlocks);
%     thisMessage2= sprintf('Please actively use the cue to ignore the distractor location');
...
%     Screen('DrawText',window,thisMessage2,xCenter-round(textBounds(3)/2),yCenter,white);
    Screen('DrawText',window,thisMessage3,xCenter-round(textBounds(3)/2),yCenter,white);
    [normBoundsRect]=Screen('TextBounds',window,'Please press space bar to begin...');
    Screen('DrawText',window,'Please press the space bar again',xCenter-round(normBoundsRect(3)/2),yCenter+150, white,gray);
    Screen('DrawingFinished',window,0);
    Screen('Flip',window);
...
%         trialMatrixCheck=[targ;dist;congMatrix;tOrient;dOrient;trialType]';
        ...
        thisTarg=thisTrialMatrix(i,1);
        thisDist=thisTrialMatrix(i,2);
        thisCong=thisTrialMatrix(i,3);
        thisTOrient=thisTrialMatrix(i,4);
        thisDOrient=thisTrialMatrix(i,5);
        thisTrialType=thisTrialMatrix(i,6);
        ...
        while keepchecking == 1;
            ...
            if x>50 && y>50
                ...
                Screen('DrawText', window, 'o', x, y,green);
            end
        ...
        cueListRB=[233 0 0; 17 103 241];
        newOrder1=randperm(size(cueListRB,1));
        cueListRB= cueListRB(newOrder1,:);
        
        
        cueList=[cueListRB(1,:);122 122 0; 22 128 109;146 111 16; 63 129 45; 140 111 78];
        newOrder2=randperm(size(cueList,1));
        cueList=cueList(newOrder2,:);
        ...
        colorListTrial=[color1;color2;color3;color4;color5;color6];
        ...
        if brokenTrial==1
            ...
            thisTrialMatrix = [thisTrialMatrix; 0 0 0 0 0 0 ]; %adding a row to the trialMatrix
        ...
        trialLog.trialInfo(i+trialCounter).cueColor=colorListTrial;
    ...
    save(sprintf('s%02d_mainBaselineTFexp', sjNum),'trialLog');
